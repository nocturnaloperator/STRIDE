...
class TrackingNotifier extends Notifier<Activity> {
  ...
  /// Requests permission and starts recording. Throws
  /// [LocationServiceException] if location isn't available — the
  /// screen should wrap this in try/catch; the message is already
  ...
  Future<void> start() async {
    ...
    _ticker = Timer.periodic(const Duration(seconds: 1), (_) {
      // Guards against the rare tick that lands the same instant as a
      // pause — state is the source of truth, not an assumption.
  ...
  void pause() {
    ...
    state = state.copyWith(status: ActivityStatus.paused);
    _positionSub?.pause(); // actually stops GPS polling — real battery savings
  }
 
  void resume() {
    if (state.status != ActivityStatus.paused) return;
    state = state.copyWith(status: ActivityStatus.recording);
    _positionSub?.resume();
  }
 
  /// Ends the activity, stops GPS/timer, and hands back the completed
  /// [Activity] for the caller to save (history list, backend, wherever).
  /// Resets local state to idle so the screen is ready for the next run.
  Activity finish() {
