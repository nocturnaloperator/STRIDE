...
class _TrainingPlanScreenState
    extends ConsumerState<TrainingPlanScreen> {
  ...
  Future<void> _generatePlan() async {
    ...
    final targetText =
        _targetTimeController.text.trim();

    if (targetText.isNotEmpty) {
      targetTimeMinutes =
          _parseTargetTime(targetText);

      if (targetTimeMinutes == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Enter target time as HH:MM or MM:SS.',
            ),
          ),
        );
        return;
    ...
    return ListView(
      ...
      children: [
        ...
        DropdownButtonFormField<TrainingGoal>(
          ...
          items: const [
            DropdownMenuItem(
              value: TrainingGoal.fiveK,
              child: Text('5K'),
            ),
            DropdownMenuItem(
              value: TrainingGoal.tenK,
              child: Text('10K'),
            ),
            DropdownMenuItem(
...
class _PlanView extends ConsumerWidget {
  ...
  Widget build(
    ...
    final suggestion =
        ref.watch(suggestedWorkoutProvider);
